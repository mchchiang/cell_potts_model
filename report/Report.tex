...
%                       
\begin{abstract}
This project investigates the existence of jamming, or the fluid-to-solid transition, in the cellular Potts model (CPM) by systematically varying two of the model's parameters: the surface energy ($\alpha$) and the cell motility strength ($P$). From analysing the mean square displacement (MSD), non-Gaussian parameter, diffusivity and MSD exponent of the cells, the model was found to exhibit jamming: an increase in surface energy changes the system's behaviour from fluid to solid-like, and an increase in the motility reduces the system's rigidity. From calculating the MSD exponents, a transition line which separates the fluid and solid-like behaviour of the system was obtained within the studied parameter space. 
\end{abstract}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figure/A2_Alpha_All_new.pdf}
